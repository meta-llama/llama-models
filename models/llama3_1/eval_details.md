...
For the pre-trained and post-trained models we use a 5-shot config with CoT prompt. We ask the model to generate the reasoning and the corresponding best choice character. The max generation length is 512 tokens for pre-trained setup and 1024 for post-trained setup.
...
For both pre-trained and post-trained models, we use a 0-shot config and report pass@1 scores. The max generation length is 1024 tokens.
