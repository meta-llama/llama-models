...
class TestTextModelInference(unittest.TestCase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                ...
                [
                    SystemMessage(content="Always answer with emojis"),
                    UserMessage(content="How to go from Beijing to NY?"),
                ],
            ],
            [
                [
                    SystemMessage(content="Always answer in riddles"),
                    UserMessage(content="What has keys but can't open locks?"),
                ]
            ],
        ],
    )
    def test_run_generation(self, dialogs):
    ...
    def test_empty_dialog(self):
        with self.assertRaises(ValueError):
            self.__class__.generator.chat_completion([], temperature=0)
